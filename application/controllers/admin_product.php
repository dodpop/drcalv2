...
class Admin_product extends MY_AdminLogin_Controller {
    ...
    function index()
    {
          $all_data = $this->admin_product_model->get_product();
          $data_['segment4'] = $this->uri->segment(4);
          // print_r($all_data);
         $data_['all_data'] = $all_data;
         $content = $this->load->view('admin/product/index', $data_, true);
         $data_['content'] = $content;
         $data_['activemenu'] = "product";
    ...
    function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
          ...
          if ($last_id > 0){
            ...
            $last_id = $this->admin_product_model->store_product($data_to_store);
          }
                ...
                if($last_id){
                    ...
                    $config['file_name'] = 'product_img_'.$new_name;
            ...
            //load the view
            $data_['segment4'] = '';
            $content = $this->load->view('admin/product/add', $data_, true);
            $data_['content'] = $content;
            $data_['activemenu'] = "product";
            $this->load->view('admin/template', $data_);
        }
    ...
    function update()
    {
        ...
        $all_data = $this->admin_product_model->get_product_by_id($id);
        $data_['all_data'] = $all_data;
